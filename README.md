...
A 27M-parameter knowledge-graph embedding — unit-norm entity vectors,
small free 4×4 block operators for relations (written in complex
arithmetic; an equal-parameter real version scores the same) — plus four
...
MIT — see [`LICENSE`](LICENSE). The checkpoints in the release are
