...
def score_batch(model, src, rel, dst, negs):
    ...
    l_neg = torch.real(zo @ model.E[negs].conj().t()) * tau
...
def main():
    ...
    opt = torch.optim.Adam([q for m in peers for q in m.parameters()],
                           lr=args.lr)
    if args.sched == "onecycle":
        sched = torch.optim.lr_scheduler.OneCycleLR(
            opt, max_lr=args.max_lr, total_steps=args.steps,
            pct_start=0.1)
    else:
    ...
    for step in range(start_step + 1, args.steps + 1):
        ...
        if args.n3 > 0:
            ...
            e_src = model.E[torch.from_numpy(src).to(dev)]
        ...
        if step % 5000 == 0 and step < args.steps:
            torch.save({"model": model.state_dict(), "step": step,
                        ...
                        "opt": opt.state_dict(),
                        "sched": sched.state_dict()}, args.save + ".ckpt")

    if not args.eval_only:
        torch.save({"model": model.state_dict(), "offset": offset,
