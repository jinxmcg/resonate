...
OUT=${OUT:-runs}
mkdir -p "$CACHE" "$OUT"
# receipt names: sparse row A/B share the "single" prefix, T=1 students "dist", T=2 students "distT2"
row_out() { case "$ROW" in sparse) echo single;; distT1) echo dist;; *) echo "$ROW";; esac; }
...
case "$ROW" in
  ...
  # sparse-shell ladder (results/sparse): row A models converted by sparse_to_dense.py,
  # T=1 students, T=2 students -- the member names below are the ones in the receipts
  sparse) model_tag() { echo "sparse_s$1"; }
          feat_tag()  { echo "analogy_sparse_s$1"; }
          pure_log()  { echo "$OUT/pure_single_s$1.log"; } ;;
  distT1) model_tag() { echo "dist_sparse_s$1"; }
          feat_tag()  { echo "analogy_dist_sparse_s$1"; }
          pure_log()  { echo "$OUT/pure_dist_s$1.log"; } ;;
  distT2) model_tag() { echo "dist_T2_s$1"; }
          feat_tag()  { echo "analogy_dist_T2_s$1"; }
          pure_log()  { echo "$OUT/pure_distT2_s$1.log"; } ;;
  *) echo "ROW must be single, dist, sparse, distT1 or distT2" >&2; exit 2 ;;
esac
...
for s in $SEEDS; do
  ...
  uv run python freeze_test.py --members $members --norm z --min-rows 4000 \
      ...
      --out "$OUT/frozen_$(row_out)_s$s.npz" \
      --result "$OUT/committed_$(row_out)_s$s.json" \
      2>&1 | tee "$OUT/committed_$(row_out)_s$s.log"
done
