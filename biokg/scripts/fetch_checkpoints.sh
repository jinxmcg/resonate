...
#   scripts/fetch_checkpoints.sh compact    # the compact row C': tiered_T2_s*.pt (10 x 39 MB), verify with verify_compact.py
...
case "$what" in
  ...
  compact) TAG=${TAG:-v2.1-compact};    dir=checkpoints;          pats=("tiered_T2_s*.pt") ;;
  ...
  *) echo "usage: $0 [sparse|compact|dense|wikikg2]" >&2; exit 2 ;;
esac
mkdir -p "$dir"
