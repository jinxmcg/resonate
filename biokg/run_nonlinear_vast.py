...
# explicit = true
# ///
"""NL1 isolated uv CUDA environment, no automatic model/data download."""
...
def main():
    ...
    for flag in ('check-environment', 'tests', 'smoke', 'evaluate', 'audit'):
    ...
    if not torch.cuda.is_available() or '5090' not in torch.cuda.get_device_name() or torch.version.cuda != '12.8':
        raise RuntimeError('Expected RTX 5090 with pinned CUDA 12.8 wheel')
    ...
    if args.tests:
        command = [sys.executable, '-m', 'unittest', 'biokg.test_nonlinear_operator', 'biokg.test_masked_neighborhood']
    else:
        command = [sys.executable, '-m', 'biokg.nonlinear_operator']
