...
def build_features(model, train, valid, offset, counts, output, log=lambda **x: None):
    # Canonicalize once, not once per query source in candidates_for().
    valid = dict(valid, head_type=np.asarray(valid["head_type"]),
                 tail_type=np.asarray(valid["tail_type"]))
